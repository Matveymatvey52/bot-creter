...
async def generate_bot_code(requirements_summary: str) -> tuple[str, dict | None, dict | None]:
    ...
    same as a template with no miniapp_config/office_hook_config today.
    ...
    they run concurrently via asyncio.gather rather than sequentially —
    review found that three serial awaits sharing ONE caller-side timeout
    (handlers/create_bot.py's asyncio.wait_for(..., timeout=360.0) /
...
async def _generate_bot_code_inner(requirements_summary: str) -> str:
    ...
    if len(templates) == 1:
        ...
        if "asyncio.run(main())" in code:
            ...
            # template-derived file (its # TEMPLATE: marker survived
            # customization, so _needs_from_scratch_wiring finds
            # config_from_bot_row/ConfigMiddleware/on_office_event already
            # present via the template's own definitions) — but
            # CUSTOMIZE_TEMPLATE_PROMPT never GUARANTEES the marker/exports
            # survive an LLM rewrite, so this is not a redundant safety net,
            # it's the same protection the from-scratch path gets.
            return append_from_scratch_registry_wiring(code)
