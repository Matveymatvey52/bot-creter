...
- "addLabel": the ready-made phrase the create screen shows, in the bot's language and in the singular accusative — "Добавить тур", "Добавить гостя", "Add a booking". Always provide it: it cannot be derived from "title" ("Туры" gives no way to reach "тур" automatically), and without it the user reads "Новая запись · Раздел «Туры»", which is machine-speak.
- "totals": [{{"field": <a numeric column>, "label": <short unit, e.g. "₽">, "signBy": {{"field": <the column that says income vs expense>, "positive": <the value meaning income>}}}}] on a resource that holds money movements. Only declare columns worth summing — a blanket sum over every numeric field would add rubles to dollars and to foreign ids. Omit "signBy" when the numbers carry no direction; then a single "Итого" tile is shown instead of "Приход / Расход / Остаток"."""
