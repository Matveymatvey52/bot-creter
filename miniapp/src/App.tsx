...
export default function App() {
  ...
  useEffect(() => {
    ...
    webApp?.ready()
    webApp?.expand()
    // Гасим нативную кнопку явно, а не просто «не показываем». Telegram держит
    // её состояние в сессии мини-аппа: если она была показана прежней версией
    // приложения, при обновлении кода она так и останется висеть синим поверх
    // страницы — код, который её не трогает, её и не уберёт. Приложение своей
    // кнопкой отправки пользуется всегда, так что гасить безопасно.
