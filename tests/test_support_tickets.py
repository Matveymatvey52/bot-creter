"""support_tickets template — miniapp_config schema-drift check.
...
Run with: python -m unittest tests.test_support_tickets
"""
...
class SupportTicketsMiniAppConfigTests(unittest.IsolatedAsyncioTestCase):
    def test_miniapp_config_resource_names(self):
        names = {r["name"] for r in support_tickets.miniapp_config["resources"]}
        self.assertEqual(names, {"tickets", "kb_articles"})

    async def test_miniapp_config_fields_match_real_schema(self):
        with tempfile.TemporaryDirectory() as tmp:
            db_path = str(Path(tmp) / "schema_check.db")
            await support_tickets.init_db(db_path)
            async with aiosqlite.connect(db_path) as db:
                for resource in support_tickets.miniapp_config["resources"]:
