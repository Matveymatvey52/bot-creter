...
class TouristDocumentsAdminBootstrapSecurityTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_non_owner_messaging_first_does_not_become_admin(self):
        config = td.config_from_bot_row(
            {"bot_id": 918, "name": "tourist_bot_owned", "display_name": None,
             "group_chat_id": None, "owner_telegram_id": 12345},
            self.data_dir,
        )
        await td.init_db(config.db_path)
        ...
        self.assertEqual(td._load_admins(config.admins_file), set())
        self.assertFalse(td._is_admin(CLIENT_ID, config))

        await dp.feed_webhook_update(bot, _text_update(2, 12345, "/start"))
        self.assertTrue(td._is_admin(12345, config))
        self.assertEqual(td._load_admins(config.admins_file), {"12345"})

    async def test_owner_is_always_admin_even_with_stale_admins_file(self):
        config = td.config_from_bot_row(
            {"bot_id": 919, "name": "tourist_bot_owned_2", "display_name": None,
             "group_chat_id": None, "owner_telegram_id": 777},
            self.data_dir,
        )
        await td.init_db(config.db_path)
        td._save_admins(config.admins_file, {"999999"})  # some other id, not the owner
        self.assertTrue(td._is_admin(777, config))  # owner: always admin
        self.assertTrue(td._is_admin(999999, config))  # still honors the file's own admin
        self.assertFalse(td._is_admin(4242, config))  # neither owner nor in the file

    async def test_bootstrap_admin_syncs_to_central_bot_admins_table(self):
        config = td.config_from_bot_row(
            {"bot_id": 920, "name": "tourist_bot_synced", "display_name": None, "group_chat_id": None},
            self.data_dir,
        )
        await td.init_db(config.db_path)
        bot, dp = _build_bot_dispatcher(config)
        ...
        central_admins = await db_module.get_bot_admins(920)
