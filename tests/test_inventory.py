"""inventory template — miniapp_config schema-drift check.
...
Run with: python -m unittest tests.test_inventory
"""
...
class InventoryMiniAppConfigTests(unittest.IsolatedAsyncioTestCase):
    def test_miniapp_config_resource_names(self):
        names = {r["name"] for r in inventory.miniapp_config["resources"]}
        self.assertEqual(names, {"items", "suppliers", "stock_movements"})

    async def test_miniapp_config_fields_match_real_schema(self):
        with tempfile.TemporaryDirectory() as tmp:
            db_path = str(Path(tmp) / "schema_check.db")
            await inventory.init_db(db_path)
            async with aiosqlite.connect(db_path) as db:
                for resource in inventory.miniapp_config["resources"]:
