...
- owner and client both see every row of the read-only "tours_public"
  resource (browsing available tours is meant for everyone).
- a user with no tour_access row at all (never messaged the bot) gets
  default_deny's empty list, not an unfiltered/leaked view.
- the separate, role_filter-less "tours" resource (owner CRUD) still 403s a
  client — role_filter was added ONLY to tours_public, not to it.
...
class TourOperatorRoleFilterTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_owner_sees_every_tour_via_tours_public(self):
        resp = await self._get("tours_public", OWNER_ID)
        self.assertEqual(resp.status, 200)
        body = await resp.json()
        self.assertEqual(len(body["items"]), 2)

    async def test_client_sees_every_tour_via_tours_public(self):
        resp = await self._get("tours_public", CLIENT_A_ID)
        self.assertEqual(resp.status, 200)
        body = await resp.json()
        self.assertEqual(len(body["items"]), 2)

    async def test_stranger_with_no_tour_access_row_gets_empty_list(self):
        resp = await self._get("tours_public", CLIENT_B_ID)
        self.assertEqual(resp.status, 200)
        body = await resp.json()
        self.assertEqual(body["items"], [])

    async def test_client_cannot_create_via_tours_public(self):
        # tours_public is creatable: False regardless of role.
        with patch.dict(os.environ, {"MINIAPP_SECRET": "s3cret"}):
            qs = f"token={mint_magic_link_token(KNOWN_BOT_ID, CLIENT_A_ID)}"
            resp = await self.client.post(
                f"/api/{KNOWN_BOT_ID}/tours_public?{qs}", json={"name": "Forged Tour"},
            )
