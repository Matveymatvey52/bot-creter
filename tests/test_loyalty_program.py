"""loyalty_program template — miniapp_config schema-drift check.
...
Run with: python -m unittest tests.test_loyalty_program
"""
...
from templates import loyalty_program


class LoyaltyProgramMiniAppConfigTests(unittest.IsolatedAsyncioTestCase):
    def test_miniapp_config_resource_names(self):
        names = {r["name"] for r in loyalty_program.miniapp_config["resources"]}
        self.assertEqual(names, {"clients", "point_entries"})

    async def test_miniapp_config_fields_match_real_schema(self):
        with tempfile.TemporaryDirectory() as tmp:
            db_path = str(Path(tmp) / "schema_check.db")
            await loyalty_program.init_db(db_path)
            async with aiosqlite.connect(db_path) as db:
                for resource in loyalty_program.miniapp_config["resources"]:
