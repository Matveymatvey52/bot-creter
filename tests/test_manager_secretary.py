"""manager_secretary template — miniapp_config schema-drift check.
...
Run with: python -m unittest tests.test_manager_secretary
"""
...
class ManagerSecretaryMiniAppConfigTests(unittest.IsolatedAsyncioTestCase):
    def test_miniapp_config_resource_names(self):
        names = {r["name"] for r in manager_secretary.miniapp_config["resources"]}
        self.assertEqual(names, {"leads", "faqs"})

    async def test_miniapp_config_fields_match_real_schema(self):
        with tempfile.TemporaryDirectory() as tmp:
            db_path = str(Path(tmp) / "schema_check.db")
            await manager_secretary.init_db(db_path)
            async with aiosqlite.connect(db_path) as db:
                for resource in manager_secretary.miniapp_config["resources"]:
