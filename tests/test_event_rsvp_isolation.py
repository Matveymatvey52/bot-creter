"""Admin-bootstrap security tests for templates/event_rsvp.py — same
...
Run with: python -m unittest tests.test_event_rsvp_isolation
"""
...
from templates import event_rsvp as ev
...
def _build_bot_dispatcher(config: ev.EventRsvpConfig) -> tuple[Bot, Dispatcher]:
    bot = Bot(token=FAKE_TOKEN)
    dp = Dispatcher(storage=MemoryStorage())
    dp.update.outer_middleware(ev.ConfigMiddleware(config))
    dp.include_router(get_template_router("event_rsvp"))
    return bot, dp


class EventRsvpAdminBootstrapTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_non_owner_messaging_first_does_not_become_admin(self):
        ...
        config = ev.config_from_bot_row(
            {"bot_id": 601, "name": "rsvp_bot_owned", "display_name": None,
             "group_chat_id": None, "owner_telegram_id": 12345},
            self.data_dir,
        )
        await ev.init_db(config.db_path)
        ...
        self.assertEqual(ev._load_admins(config.admins_file), set())
        self.assertFalse(ev._is_admin(CLIENT_ID, config))

        await dp.feed_webhook_update(bot, _text_update(2, 12345, "/start"))
        self.assertTrue(ev._is_admin(12345, config))
        self.assertEqual(ev._load_admins(config.admins_file), {"12345"})
    ...
    async def test_owner_is_always_admin_even_with_stale_admins_file(self):
        ...
        config = ev.config_from_bot_row(
            {"bot_id": 602, "name": "rsvp_bot_owned_2", "display_name": None,
             "group_chat_id": None, "owner_telegram_id": 777},
            self.data_dir,
        )
        await ev.init_db(config.db_path)
        ev._save_admins(config.admins_file, {"999999"})  # some other id, not the owner
        self.assertTrue(ev._is_admin(777, config))  # owner: always admin
        self.assertTrue(ev._is_admin(999999, config))  # still honors the file's own admin
        self.assertFalse(ev._is_admin(4242, config))  # neither owner nor in the file
    ...
    async def test_standalone_mode_keeps_first_comer_bootstrap(self):
        ...
        owner to defer to."""
        config = ev.config_from_bot_row(
            {"bot_id": 603, "name": "rsvp_bot_standalone", "display_name": None, "group_chat_id": None},
            self.data_dir,
        )
        await ev.init_db(config.db_path)
        bot, dp = _build_bot_dispatcher(config)
        ...
        self.assertEqual(ev._load_admins(config.admins_file), {"111"})
        self.assertTrue(ev._is_admin(FIRST_USER, config))
    ...
    async def test_bootstrap_admin_syncs_to_central_bot_admins_table(self):
        ...
        config = ev.config_from_bot_row(
            {"bot_id": 604, "name": "rsvp_bot_synced", "display_name": None, "group_chat_id": None},
            self.data_dir,
        )
        await ev.init_db(config.db_path)
        bot, dp = _build_bot_dispatcher(config)
    ...
    async def test_configs_point_to_different_files(self):
        config_a = ev.config_from_bot_row(
            {"bot_id": 701, "name": "rsvp_bot_a", "display_name": None, "group_chat_id": None}, self.data_dir
        )
        config_b = ev.config_from_bot_row(
            {"bot_id": 702, "name": "rsvp_bot_b", "display_name": None, "group_chat_id": None}, self.data_dir
        )
