...
class DiscoverTemplatesRegressionOnRealTemplates(unittest.TestCase):
    ...
    event_manager, debtors, and vehicle_service were each added after this
    test was first written — every new template added purely by dropping a
    file into templates/, with zero changes to claude_service.py, needs this
    ...
    def test_all_real_templates_are_discovered(self):
        ...
        self.assertEqual(
            names,
            {"accountant", "booking_beauty", "booking_fitness", "booking_medical", "campaign_tracker", "debtors", "event_manager", "expense_tracker", "inventory", "manager_secretary", "moderator", "orders_tracker", "referral_program", "shop_catalog", "tour_operator", "tourist_documents", "trip_manager", "vehicle_service"},
        )
