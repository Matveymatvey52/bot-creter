...
class DiscoverTemplatesRegressionOnRealTemplates(unittest.TestCase):
    ...
    that must not affect TEMPLATE:/USE FOR: parsing). booking_medical and
    event_manager were each added after this test was first written — every
    ...
    def test_all_real_templates_are_discovered(self):
        ...
        self.assertEqual(
            names,
            {"accountant", "booking_beauty", "booking_fitness", "booking_medical", "campaign_tracker", "event_manager", "inventory", "manager_secretary", "moderator", "orders_tracker", "referral_program", "shop_catalog", "tour_operator", "tourist_documents", "trip_manager"},
        )
    ...
    def test_prompt_lists_all_real_templates(self):
        prompt = claude_service._build_template_select_prompt()
        for name in ("accountant", "booking_beauty", "booking_fitness", "booking_medical", "campaign_tracker", "inventory", "manager_secretary", "moderator", "orders_tracker", "referral_program", "shop_catalog", "tour_operator", "tourist_documents", "trip_manager"):
