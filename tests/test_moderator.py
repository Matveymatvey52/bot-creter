"""moderator template — miniapp_config schema-drift check.
...
Only moderation_log is declared as a resource — every other table in this
template (chat_settings, stopwords, warnings, known_groups,
dm_fallback_sent, bot_admins) has a composite or non-`id` primary key, which
runtime/miniapp_api.py's generic handlers don't support.

Run with: python -m unittest tests.test_moderator
"""
...
class ModeratorMiniAppConfigTests(unittest.IsolatedAsyncioTestCase):
    def test_miniapp_config_resource_names(self):
        names = {r["name"] for r in moderator.miniapp_config["resources"]}
        self.assertEqual(names, {"moderation_log"})

    async def test_miniapp_config_fields_match_real_schema(self):
        with tempfile.TemporaryDirectory() as tmp:
            db_path = str(Path(tmp) / "schema_check.db")
            await moderator.init_db(db_path)
            async with aiosqlite.connect(db_path) as db:
                for resource in moderator.miniapp_config["resources"]:
                    ...
                    self.assertIn("id", real_columns, f"{resource['name']}: table has no single `id` PK")
