"""referral_program template — miniapp_config schema-drift check.
...
Only `referrals` is declared as a resource — `participants` uses `user_id`
as its primary key (no `id` column), which runtime/miniapp_api.py's generic
handlers don't support.

Run with: python -m unittest tests.test_referral_program
"""
...
class ReferralProgramMiniAppConfigTests(unittest.IsolatedAsyncioTestCase):
    def test_miniapp_config_resource_names(self):
        names = {r["name"] for r in referral_program.miniapp_config["resources"]}
        self.assertEqual(names, {"referrals"})

    async def test_miniapp_config_fields_match_real_schema(self):
        with tempfile.TemporaryDirectory() as tmp:
            db_path = str(Path(tmp) / "schema_check.db")
            await referral_program.init_db(db_path)
            async with aiosqlite.connect(db_path) as db:
                for resource in referral_program.miniapp_config["resources"]:
