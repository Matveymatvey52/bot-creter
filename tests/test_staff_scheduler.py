"""staff_scheduler template — miniapp_config schema-drift check.
...
Run with: python -m unittest tests.test_staff_scheduler
"""
...
from templates import staff_scheduler


class StaffSchedulerMiniAppConfigTests(unittest.IsolatedAsyncioTestCase):
    def test_miniapp_config_resource_names(self):
        names = {r["name"] for r in staff_scheduler.miniapp_config["resources"]}
        self.assertEqual(names, {"employees", "shifts"})

    async def test_miniapp_config_fields_match_real_schema(self):
        with tempfile.TemporaryDirectory() as tmp:
            db_path = str(Path(tmp) / "schema_check.db")
            await staff_scheduler.init_db(db_path)
            async with aiosqlite.connect(db_path) as db:
                for resource in staff_scheduler.miniapp_config["resources"]:
