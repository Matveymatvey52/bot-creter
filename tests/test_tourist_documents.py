"""tourist_documents template — miniapp_config schema-drift check.
...
Run with: python -m unittest tests.test_tourist_documents
"""
...
from templates import tourist_documents


class TouristDocumentsMiniAppConfigTests(unittest.IsolatedAsyncioTestCase):
    def test_miniapp_config_resource_names(self):
        names = {r["name"] for r in tourist_documents.miniapp_config["resources"]}
        self.assertEqual(names, {"tourists", "tour_groups"})

    async def test_miniapp_config_fields_match_real_schema(self):
        with tempfile.TemporaryDirectory() as tmp:
            db_path = str(Path(tmp) / "schema_check.db")
            await tourist_documents.init_db(db_path)
            async with aiosqlite.connect(db_path) as db:
                for resource in tourist_documents.miniapp_config["resources"]:
