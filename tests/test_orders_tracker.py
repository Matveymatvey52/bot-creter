"""orders_tracker template — miniapp_config schema-drift check.
...
Run with: python -m unittest tests.test_orders_tracker
"""
...
class OrdersTrackerMiniAppConfigTests(unittest.IsolatedAsyncioTestCase):
    def test_miniapp_config_resource_names(self):
        names = {r["name"] for r in orders_tracker.miniapp_config["resources"]}
    ...
    async def test_miniapp_config_fields_match_real_schema(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            await orders_tracker.init_db(db_path)
            async with aiosqlite.connect(db_path) as db:
                for resource in orders_tracker.miniapp_config["resources"]:
