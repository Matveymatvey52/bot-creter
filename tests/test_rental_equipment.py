"""rental_equipment template — miniapp_config schema-drift check.
...
Run with: python -m unittest tests.test_rental_equipment
"""
...
from templates import rental_equipment
...
class RentalEquipmentMiniAppConfigTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_miniapp_config_fields_match_real_schema(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            await rental_equipment.init_db(db_path)
            async with aiosqlite.connect(db_path) as db:
                for resource in rental_equipment.miniapp_config["resources"]:
