...
class BookingFitnessAdminSecurityBootstrapTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_non_owner_messaging_first_does_not_become_admin(self):
        config = bf.config_from_bot_row(
            {"bot_id": 611, "name": "fit_bot_owned", "display_name": None,
             "group_chat_id": None, "owner_telegram_id": 12345},
            self.data_dir,
        )
        await bf.init_db(config.db_path)
        ...
        self.assertEqual(bf._load_admins(config.admins_file), set())
        self.assertFalse(bf._is_bot_admin(CLIENT_ID, config))

        await dp.feed_webhook_update(bot, _text_update(2, 12345, "/start"))
        self.assertTrue(bf._is_bot_admin(12345, config))
        self.assertEqual(bf._load_admins(config.admins_file), {"12345"})

    async def test_owner_is_always_admin_even_with_stale_admins_file(self):
        config = bf.config_from_bot_row(
            {"bot_id": 612, "name": "fit_bot_owned_2", "display_name": None,
             "group_chat_id": None, "owner_telegram_id": 777},
            self.data_dir,
        )
        await bf.init_db(config.db_path)
        bf._save_admins(config.admins_file, {"999999"})  # not the owner
        self.assertTrue(bf._is_bot_admin(777, config))
        self.assertTrue(bf._is_bot_admin(999999, config))
        self.assertFalse(bf._is_bot_admin(4242, config))
    ...
    async def test_standalone_mode_keeps_first_comer_bootstrap(self):
        ...
        config = bf.config_from_bot_row(
            {"bot_id": 613, "name": "fit_bot_standalone", "display_name": None, "group_chat_id": None},
            self.data_dir,
        )
        ...
        self.assertEqual(bf._load_admins(config.admins_file), {"999"})
        self.assertTrue(bf._is_bot_admin(FIRST_COMER, config))

    async def test_bootstrap_admin_syncs_to_central_bot_admins_table(self):
        config = bf.config_from_bot_row(
            {"bot_id": 614, "name": "fit_bot_synced", "display_name": None, "group_chat_id": None},
            self.data_dir,
        )
        await bf.init_db(config.db_path)
        bot, dp = _build_bot_dispatcher(config)
