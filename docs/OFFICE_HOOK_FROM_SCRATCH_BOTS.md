...
## 0. Статус: реализовано (2026-08-16)

Владелец объявил from-scratch генерацию ОСНОВНЫМ сценарием (не редким/теоретическим), поэтому вариант A выбран и реализован сразу — без ожидания живого прод-примера, вопреки первоначальному "стоп" в §4 ниже (тот стоп снят явным указанием владельца).

**Выбор: вариант A** (унификация конвенции), реализован прагматично — НЕ через LLM-промпт для всех трёх экспортов, а гибридно:
- `services/claude_service.py`: `GENERATE_SYSTEM_PROMPT` теперь требует `async def init_db(db_path: str) -> None` (раньше — не гарантировано вообще, `BOOKING_EXTRA` требовал `init_db()` без параметра). Это единственный экспорт, для которого нужно LLM-знание конкретных таблиц бота.
- `config_from_bot_row`/`ConfigMiddleware`/`on_office_event` — стопроцентно детерминированный boilerplate (подтверждено: идентичен во всех templates/*.py), поэтому НЕ отдан LLM — программно дописывается функцией `append_from_scratch_registry_wiring()` после генерации (и после всех review-проходов, т.к. они не знают об этом блоке и могли бы его "исправить"/уронить).
- `runtime/registry.py`'s `build_entry()`: получил `file_path`-параметр и новую функцию `_load_generated_bot_module_async(bot_id, file_path)` — импортирует файл бота напрямую (`importlib.util.spec_from_file_location`), а не через `templates.<id>`, поскольку from-scratch файлы физически лежат в `data/generated_bots/`, не в пакете `templates/`. Путь пробуется ТОЛЬКО когда `template_id` не задан (не как universal catch-all при `module is None`) — иначе бот со сломанным `template_id` мог бы тихо "починиться" через файловый импорт вместо явного WARNING.
- Открытый вопрос §3 ("как from-scratch боты физически запускаются") разрешён отдельным расследованием: прод всегда в webhook-режиме (`PUBLIC_BASE_URL` задан), а `bot_runner.py`'s subprocess-модель — только для local/polling режима (`main.py`). Значит `build_entry()` — единственное и верное место для фикса, второй путь доставки не нужен.
- Миграция существующих прод-ботов: на момент проверки (см. предыдущую сессию) в проде был единственный пользовательский бот, и тот template-based — миграция не потребовалась. Уже задеплоенные from-scratch боты без этой обвязки (если появятся до следующего деплоя) получат её при следующем `cb_recreate`/regenerate, не автоматически.

Тесты: `tests/test_from_scratch_office_hook.py` — 9 тестов, включая сквозной сценарий (from-scratch бот на диске → `build_entry()` → office event → generic-хук пишет в БД бота). Полный сьют: 1104/1104 (не считая одного известного флейки-теста по таймингу в `test_sheets_module.py`, не связанного с этим изменением).

Ревью нашло и было исправлено: (1) синтезированные (2-шаблонные) боты изначально пропускали wiring-инъекцию — `_generate_bot_code_inner` возвращал код раньше, чем доходил до общего append-шага; (2) `_needs_from_scratch_wiring` проверяла присутствие только ОДНОГО экспорта (`config_from_bot_row`), из-за чего LLM-рерайт (`improve_bot_code`/`fix_bot_code`), сохранивший этот один экспорт но уронивший `ConfigMiddleware`/`on_office_event`, тихо считался "уже wired"; (3) file_path-фоллбэк в `build_entry()` мог маскировать сломанный `template_id`, если файл случайно импортировался успешно другим путём — сужен до срабатывания только когда `template_id` не задан вовсе.
