...
# Фаза «новый шаблон: staff_scheduler» — дизайн (утверждён владельцем, код ещё не написан)

Ветка `template-staff-scheduler` (от `master`, подтянута fast-forward после мержа `template-loyalty-program`/`template-rental-equipment`), worktree `~/bot-creter-scheduler`. Дизайн ниже утверждён владельцем явным «ок» — записан здесь ПОСЛЕ утверждения, чтобы следующая сессия могла перечитать его из файла, а не восстанавливать по памяти чата.

## Доменное содержание (владельцем, не переосмыслять)

Расписание работы конкретных сотрудников — НЕ путать с `booking_fitness` (расписание занятий с местами для клиентов):
- Сотрудник = имя, должность (опционально), контакт.
- Смена = сотрудник + дата + время начала/конца + опциональная заметка.
- Обзор на текущую и следующую неделю — по каждому сотруднику или по дням.
- Пересекающиеся смены одного сотрудника — при добавлении новой явное предупреждение, не тихая запись.
- Это **внутренний инструмент для админа** — сотрудники ботом сами не пользуются (ближайший образец по структуре — `debtors.py`: только приватный чат, только `admins_file`-админы, без клиентской ветки).

## Config — типовой каркас, без доменных полей
...
class StaffSchedulerConfig:
    ...
    group_chat_id: str | None = None
```
`config_from_env`/`config_from_bot_row`/`ConfigMiddleware` — идентичны остальным шаблонам (bot_id-based пути).
...
CREATE TABLE IF NOT EXISTS employees (
    id       INTEGER PRIMARY KEY AUTOINCREMENT,
    name     TEXT NOT NULL,
    position TEXT,              -- опционально, по заданию
    contact  TEXT NOT NULL,
    active   INTEGER NOT NULL DEFAULT 1   -- "удалённый" сотрудник = active=0, не hard-delete
);
...
CREATE TABLE IF NOT EXISTS shifts (
    ...
    end_time    TEXT NOT NULL,   -- 'HH:MM'; end_time <= start_time = смена уходит за полночь
    note        TEXT,
    created_at  TEXT DEFAULT (datetime('now','localtime'))
);
...
**Soft-delete у сотрудника, не hard-delete**: смены ссылаются на `employee_id` — удаление строки сотрудника либо роняет FK, либо тихо обнуляет историю смен. `active=0` прячет сотрудника из пикера «добавить смену», но его прошлые смены остаются видны в недельном/дневном обзоре и в «Расписание по сотруднику».

**Пересекающиеся смены за полночь**: ночная смена (например, 22:00–06:00) хранится как `end_time <= start_time`. Конфликт-проверка нормализует обе смены (новую и уже сохранённые) во «внутреннюю» временную ось, якорь — начало более раннего из двух календарных дней в паре, и сравнивает интервалы `[start, end)` формулой `start1 < end2 AND start2 < end1`. Проверяются смены ЭТОГО ЖЕ сотрудника с `shift_date` в {D-1, D}, где D — дата новой смены (смена, начавшаяся накануне и ушедшая за полночь, может пересекаться со сменой, начинающейся в D).

## Флоу

Главное меню (reply-кнопки, приватный чат, только для `admins_file`-админов):
- ➕ Добавить смену
- 📅 Расписание на неделю (по дням)
- 🧑 Расписание по сотруднику
- 👥 Сотрудники
- 👤 Админы бота

**Окно «текущая + следующая неделя»**: понедельник текущей ISO-недели → +13 дней (14 дней, 2 полных недели Пн–Вс). Одно и то же окно используется и для просмотра по дням, и как ограничение выбора даты в «Добавить смену» (кнопки, без свободного ввода даты — по правилу дизайна «только кнопки/FSM»).

**➕ Добавить смену** (FSM `ShiftFlow`): `employee` (кнопки активных сотрудников; если пусто — подсказка сперва завести сотрудника) → `date` (14 кнопок дней окна, метки в стиле `_date_label` из `booking_fitness.py`) → `start` (кнопки из CUSTOMIZE-константы `SHIFT_TIME_OPTIONS`, по умолчанию почасовая сетка 06:00–23:00) → `end` (та же сетка, БЕЗ фильтрации «после start» — ночные смены легальны) → `note` (свободный текст + кнопка «⏭ Без заметки») → `confirm` (карточка «сотрудник / дата / время / заметка» + «✅ Подтвердить» / «❌ Отмена»). **Если найдено пересечение** — та же карточка `confirm` дополнительно показывает блок `⚠️ Пересекается со сменой: <сотрудник>, <дата>, <время>`, кнопка подтверждения меняется на «⚠️ Всё равно добавить» — предупреждение всегда на экране подтверждения, тихой записи в обход него нет.

**📅 Расписание на неделю (по дням)**: 14 кнопок-дней → тап по дню → список смен этого дня (сотрудник, время, превью заметки), отсортирован по `start_time` → тап по смене → карточка деталей + «🗑 Удалить» (с подтверждением).

**🧑 Расписание по сотруднику**: список сотрудников (включая неактивных — для просмотра истории) → тап → хронологический список его смен в окне 14 дней → тап по смене → та же карточка удаления.

**👥 Сотрудники**: список активных (➖ на удаление, = `active=0`) + «➕ Добавить сотрудника». Добавление (FSM `EmployeeFlow`): `name` → `position` (кнопка «⏭ Без должности») → `contact` → сохранить.

**👤 Админы бота**: стандартный `AdminMgmtFlow` (add_admin/remove_admin_pick), без изменений от остальных шаблонов.
...
## Вне рамок этой задачи (сознательно)
- Сотрудники не заходят в бота сами и не видят своё расписание — только админ. Отдельная доработка при необходимости, не эта задача.
- `COMPATIBLE_WITH` в `features/sheets.py`/`features/payments.py` уже сейчас не включает `debtors`/`vehicle_service`/`rental_equipment` — известный отдельный пробел, не трогается в рамках этой задачи.

## Статус на момент записи
Дизайн утверждён владельцем («ок»). Код (`templates/staff_scheduler.py`) ещё не написан на момент записи этого раздела — следующий шаг: реализация (Task 2), затем авто-обнаружение (`discover_templates()`/`test_template_discovery.py`, без правок вне `templates/staff_scheduler.py`), затем smoke-тест изоляции + smoke на конфликт смен (Task 4), затем review-orchestrator и полный сьют (Task 5), стоп перед коммитом кода.
